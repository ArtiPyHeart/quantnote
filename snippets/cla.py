...
class CLA:
    ...
    def allocate(
        ...
        # Initial checks
        if asset_prices is None and (
            expected_asset_returns is None or covariance_matrix is None
        ):
            raise ValueError(
        ...
        while True:
            ...
            if (lambda_in is None or lambda_in < 0) and (
                lambda_out is None or lambda_out < 0
            ):
    ...
    def _init_algo(self):
        ...
        structured_array = np.zeros(
            (self.expected_returns.shape[0]), dtype=[("id", int), ("mu", float)]
        )
        expected_returns = [
            self.expected_returns[i][0] for i in range(self.expected_returns.shape[0])
    ...
    def _compute_lambda(self, covar_f_inv, covar_fb, mean_f, w_b, asset_index, b_i):
        ...
        lambda_value = float(
            ((1 - l_1 + l_2) * c_4[asset_index] - c_1 * (b_i + l_3[asset_index]))
            / c_final
        )
    ...
    def _get_bounded_weights(self, free_weights):
        ...
        return self._diff_lists(
            list(range(self.expected_returns.shape[0])), free_weights
        )
        ...
        while True:
            ...
            if abs(sum(self.weights[index_1]) - 1) > tol:
                ...
                for index_2 in range(len(self.weights[index_1])):
                    if (
                        self.weights[index_1][index_2] - self.lower_bounds[index_2]
                        < -tol
                        or self.weights[index_1][index_2] - self.upper_bounds[index_2]
                        > tol
                    ):
                        flag = True
                        break
    ...
    def _free_bound_weight(self, free_weights):
        ...
        if len(free_weights) < self.expected_returns.shape[0]:
            ...
            for i in bounded_weight_indices:
                ...
                if (
                    self.lambdas[-1] is None or lambda_i < self.lambdas[-1]
                ) and lambda_i > self._infnone(lambda_out):
                    lambda_out, i_out = lambda_i, i
        return lambda_out, i_out

    def _initialise(
        self, asset_prices, expected_asset_returns, covariance_matrix, resample_by
    ):
        ...
        if (
            ...
            == np.ones(self.expected_returns.shape) * self.expected_returns.mean()
        ).all():
        ...
        if isinstance(self.weight_bounds[0], numbers.Real):
            self.lower_bounds = (
                np.ones(self.expected_returns.shape) * self.weight_bounds[0]
            )
        else:
        ...
        if isinstance(self.weight_bounds[0], numbers.Real):
            self.upper_bounds = (
                np.ones(self.expected_returns.shape) * self.weight_bounds[1]
            )
        else:
    ...
    def _min_volatility(self):
        ...
        min_var = min(var)
        return min_var ** 0.5, self.weights[var.index(min_var)]
